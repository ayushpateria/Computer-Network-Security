...
class Firewall:
    ...
    def get_version(self, packet):
        try:
            ver = (ord(packet[0:1]) & 240) >> 4
            print(ver)
            return ver
        except struct.error:
            return None    
    ...
    def get_protocol_ipv6(self, packet):
        try:
            protocol = struct.unpack('!B', packet[6:7])
            return self.strip_format(protocol)
        except struct.error as e:
            print(e)
            return None, None
    ...
    def handle_packet(self, packet, rule):
        ...
        if (ver == 4):
            ...
            return True
        if ver == 6:
            srcip = ipaddr.IPv6Address(ipaddr.Bytes(packet[8:24]))
            dstip = ipaddr.IPv6Address(ipaddr.Bytes(packet[24:40]))
            
            protocol = self.get_protocol_ipv6(packet)
            ...
            #filter IP addresses
            if rule["sourceip"] != "any":
                if not self.is_IP_in_range(srcip, rule["sourceip"]):
                    print(Fore.YELLOW + "FILTER :: IP not in specified range" + Style.RESET_ALL)
                    return False
            #filter ports
            if ((protocol == 6) or (protocol == 17)) and ((rule["protocol"] == "all") or (rule["protocol"] == "tcp") or (rule["protocol"] == "udp")):
                if rule["sport1"]:
                    port = self.get_port(packet, 40)
                    if port == None:
                        print(Fore.YELLOW + "ERROR :: Source Port is incorrect" + Style.RESET_ALL)
                        return None
                    elif not self.is_port_in_range(port, rule["sport1"], rule["sport2"]):
                ...
                if rule["dport1"]:
                    ...
                    if port == None:
                        ...
                        return False
                        # Check for a potential port scan 
            dport = self.get_port(packet, 42)
            if rule["action"] == "ACCEPT" and not self.check_port_scan(srcip, dport, protocol):
                return False
            return True
